...
- **Standard** (a typical component change, bug fix, or refactor within one package) — run all three personas against the diff and enough surrounding context to understand it, without necessarily reading every unrelated file in the repo.
- **High-risk** (touches auth/data-access patterns, adds a new dependency, spans many files, or changes a pattern shared across Marko/React/Skin) — full depth, and explicitly note if a change in one framework layer isn't mirrored in its siblings (e.g., a new prop added to `@evo-web/react` with no corresponding change in `@evo-web/marko`, or a Skin BEM class renamed without updating the framework wrappers that reference it) — this cross-layer drift is one of the easiest things to miss in a single-file diff view and one of the most distinctive failure modes in a multi-framework monorepo like this one.
...
**Correctly flagged:** A PR adds a new API-facing method that reads a request parameter and uses it to build a filesystem path or database query with no validation. The Security Auditor states the concrete exploit path (what input, what it does); the finding stands on its own regardless of what the other two personas find.
