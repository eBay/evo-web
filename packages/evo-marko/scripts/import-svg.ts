  ...
  for (const [name, themes] of iconMap) {
    ...
    fs.writeFileSync(
      type === "image-placeholder"
        ? path.join(outputBaseDir, "evo-image-placeholder", "index.marko")
        : path.join(
            getOutputDir(type),
            `evo${type === "icon" ? "-icon" : ""}-${name}.marko`,
          ),
      `// AUTO-GENERATED by \`importSVG\` script (\`scripts/import-svg.ts\`)

import type { Input as IconInput } from "../${type === "icon" ? "" : "evo-icon/"}index.marko";
export type Input = Omit<IconInput, \`_\${string}\`>;
<evo-icon ...input _name="${name}"${size ? ' _size="' + size + '"' : ""} _type="${type}" _symbol=${JSON.stringify(themes)}/>
`,
    );
  }
}
