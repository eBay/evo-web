...
class PopperDropdown {
  ...
  init() {
    this.cleanup = autoUpdate(this.host, this.overlay, this.update.bind(this));
  }
...
// MIXED CHECKBOX
document
  .querySelectorAll('.checkbox input[aria-checked="mixed"]')
  .forEach(function (el) {
...
document.querySelectorAll(".date-textbox").forEach(function (el) {
  const popperDropdown = new PopperDropdown(el, null, ".date-textbox__popover");
...
class ProgressBarExpressive {
  constructor(widgetEl, messages) {
    this.messageContainerEl = widgetEl.querySelector(
  ...
  queueNextMessage() {
    if (this.isInitialized && this.messages.length > 1) {
      ...
      this.currentMessageEl.classList.add(
  ...
  showMessage() {
    if (this.isInitialized) {
      ...
      this.currentMessageEl.classList.remove(
        "progress-bar-expressive__message--out",
      );
      this.nextMessageEl.classList.remove(
...
document
  .querySelectorAll(".progress-bar-expressive-messages-example")
  .forEach(function (widgetEl) {
    const expressiveProgressBar = new ProgressBarExpressive(widgetEl, [
      "Hang tight.",
...
document.querySelectorAll(".tooltip").forEach(function (widgetEl) {
  if (widgetEl.classList.contains("tooltip--js")) {
    return;
  }
  pageWidgets.push(
    new Expander(widgetEl, {
...
document.querySelectorAll(".infotip").forEach(function (widgetEl) {
  if (widgetEl.classList.contains("infotip--js")) {
    return;
  }
...
document.querySelectorAll(".tourtip").forEach(function (widgetEl) {
  if (widgetEl.classList.contains("tourtip--js")) {
    return;
  }
  widgetEl
    .querySelector(".tourtip__close")
    .addEventListener("click", function () {
...
document
  .querySelectorAll(".phone-input .listbox-button")
  .forEach(function (widgetEl) {
    const options = {
      autoSelect: widgetEl.dataset.makeupAutoSelect === "true",
      buttonLabelSelector: ".btn__text",
    ...
    widgetEl.addEventListener("makeup-listbox-button-change", (e) => {
      ...
      widgetEl.nextElementSibling.querySelector(".textbox > span").textContent =
...
document
  .querySelectorAll(".switch:not(.switch--form)")
  .forEach(function (widgetEl) {
    pageWidgets.push(
      new Switch(widgetEl, {
        bem: {
...
(function () {
  ...
  document
    .querySelectorAll(sSelectorButtonGroups)
    .forEach(function (elToggleButtonGroup) {
  ...
  const sToggleButtonGroupDemosSelector =
      ".toggle-button-group-responsive-demo",
  ...
  document
    .querySelectorAll(sToggleButtonGroupDemosSelector)
...
(function () {
  ...
  const sFilePreviewCardGroupDemoSelector =
    ...
    sDemoContainerSelector = ".file-preview-card-group-breakpoints-container";
  document
    .querySelectorAll(sFilePreviewCardGroupDemoSelector)
    .forEach(function (eFilePreviewCardGroupDemo) {
      const elRange = eFilePreviewCardGroupDemo.querySelector(sRangeSelector),
        elOutput =
          eFilePreviewCardGroupDemo.querySelector(sRangeOutputSelector),
...
document.querySelectorAll(".field").forEach(function (elCharContainer) {
  ...
  if (characterCount) {
    ...
    elInput.addEventListener(
      "keydown",
      debouncedKeydown.bind(this, elInput, elMeterText),
    );
  }
});
  ...
  document
    ...
    .forEach(function (elChipsCombobox) {
      ...
      elChipsCombobox.addEventListener("click", function (event) {
        ...
        if (
          !elClicked.matches(sComboboxOptionSelector) &&
          !elClicked.matches(sChipDeleteSelector)
        )
          return;
