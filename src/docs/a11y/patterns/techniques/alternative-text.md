...
In all cases the alt attribute **must** **always** be present, but it _can_ and should have an empty value in certain situations (read on for more details). Removing the alt attribute altogether would result in the screen reader announcing the entire image url, which is not desirable for our users.
