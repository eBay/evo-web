...
Forcing links to open in a new window is an anti-pattern because we are taking control _away_ from the user and forcing web-browsing behaviour upon them. If a user wishes to open a link in a new window, they can do so by using their mouse or keyboard shortcut, without our intervention.
...
Ideally we should also warn _sighted_ users of this behaviour. We can do this by adding the text to a foreground image:
