...
The evo-web AI pipeline is a supervised, step-by-step workflow that takes a component contract written collaboratively by designers and engineers and generates a complete, production-ready component — including static HTML and CSS, Marko and React implementations, accessibility validation, storybook stories, and documentation. Engineers stay in control at every gate and review all output before anything ships.
...
**Component Spec (`*.spec.json`)** is a structured JSON file provided by the design team alongside the contract. It contains machine-readable definitions of every prop (with type, allowed values, and default), design token mappings (which CSS custom properties drive color, spacing, and typography), slot definitions, and component states. The spec is the authoritative data source for code generation — it eliminates most of the guesswork that would otherwise produce inconsistencies between design intent and output.

Both files live alongside the component's documentation in `src/routes/_index/components/<name>/` — the single folder that holds everything about a component: design inputs, approval artifacts, and generated docs pages.
...
With the approved contract and spec in hand, the AI reads both and produces two files: `manifest.json` and `gap-report.json`. The manifest is a dense, structured JSON file encoding every aspect of the component that downstream code generation needs: props, slots, states, BEM class names, ARIA attributes, keyboard behavior, accessibility obligations for consumers, and design token mappings. The gap report documents every inference the AI made and every field it could not determine with confidence, along with its reasoning.

The spec takes precedence for structured fields — props, tokens, slots, states — because it is machine-precise. The contract's prose fills in the fields the spec does not cover: accessibility rationale, keyboard interaction details, behavioral edge cases, and caller obligations. Together they give the manifest high completeness with minimal ambiguity.

---

## Gate 2 — Manifest Review

The engineer reviews `manifest.json` and `gap-report.json`. This is the last human checkpoint before any component code is written. The engineer resolves any low-confidence gaps, corrects misinterpretations, and signs off. Once approved, the orchestrator generates code purely from the manifest — it does not re-read or re-interpret the contract. This separation means the same manifest always produces the same output, regardless of when it runs.

---

## The Orchestrator

After the manifest is approved, the engineer runs `/evo-component <component-name>`. The orchestrator reads the manifest, asks which scope to run if not specified, and sequences all downstream sub-skills. Each sub-skill is given an explicit declaration from the orchestrator of what has run before it in this session — it never guesses from what's on disk, which would produce wrong results on component revisions where old files already exist.
...
The Overview tab of the docs site is written here — the landing page for the component. It includes the component banner (populated from the component registry), a brief description, and cross-links to related components where relevant. The component registry (`component-metadata.json`) is updated at this step, wiring up the Marko Storybook and React Storybook tab links. Storybook paths are set if determinable from the session; otherwise flagged as a follow-up after first deployment.
...
Not every pipeline run regenerates everything. The `--scope` flag tells the orchestrator what changed:

**`full`** (default) — A new component, or a revision significant enough to touch all layers. Runs all 16 steps.

**`static`** — The HTML structure or SCSS changed (a new BEM modifier, a layout adjustment). Runs the static layer, CSS docs, and a11y Pass 1. Does not touch Marko or React. If the static changes require framework updates, the engineer follows up with `--scope interactive`.
...
**`style`** — SCSS only (a color token update, a spacing change). HTML structure and component interfaces are unchanged. Runs only the static component (SCSS generation), updates `css+page.marko`, then builds and runs QA. Storybooks and a11y docs are untouched.

---

## Discrete Skill: /evo-icon

The icon pipeline is the one discrete skill designed as part of this system. Engineers invoke it directly — it is not part of the component generation sequence, but it can be triggered by the orchestrator when a major version icon deletion requires removing the icon from Marko and React simultaneously.
